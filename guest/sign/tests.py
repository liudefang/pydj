...
class ModelTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(id=3, name='乐视2pro发布会', status=True, limit=2000, address='北京会展中心',start_time=datetime(2017, 10,10, 15, 0, 0))
        Guest.objects.create(id=11,event_id=3,realname='alen',phone='13711001101',email='alen@mail.com', sign=False)

    def test_event_models(self):
        result = Event.objects.get(name="乐视2pro发布会")
        self.assertEqual(result.address,"深圳体育广场")
...
class LoginActionTest(TestCase):
    ...
    def setUp(self):
        User.objects.create_user('admin', 'admin@139.com', 'admin123456')
        self.c = Client()
...
class EventManageTest(TestCase):
    '''发布会管理 '''
    def setUp(self):
        Event.objects.create(id=4, name='xiaomi5', limit=2000, status=True,address='beijing', start_time=datetime(2016, 8, 10, 14, 0, 0))
        self.c = Client()
    ...
    def test_event_manage_search_Success(self):
        '''测试发布会搜索'''
        response = self.c.post('/searc_name/',{"name":"xiaomi5"})
        self.assertEqual(response.status_code,200)
        self.assertIn(b"xiaomi5",response.content)
        self.assertIn(b"beijing",response.content)
